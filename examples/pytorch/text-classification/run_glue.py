...
def main():
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        def expected_calibration_error(samples, labels, M=5):
            bin_boundaries = np.linspace(0, 1, M + 1)
            bin_lowers = bin_boundaries[:-1]
            bin_uppers = bin_boundaries[1:]

            confidences = np.max(samples.numpy(), axis=1)
            predicted_label = np.argmax(samples.numpy(), axis=1).astype(float)
            accuracies = preds == p.label_ids
        ...
        result["combined_score"] = (
            " ece: "
            + str(expected_calibration_error(probabilities, p.label_ids).item())
            + " confidences: "
            + str(np.mean(np.max(probabilities.numpy(), axis=1)).item())
        )
        return result
